...
import oop.stream.printer.ConsoleActivityPrinter;
...
public class SprintLogApp {
   ...
   public static void main(String[] args) {
       ...
       *   - false 키 → 비공개 활동 목록
       */
      Map<Boolean, List<LearningActivity>> byVisibility =
            activities
                  .stream()
                  .collect(Collectors.partitioningBy(LearningActivity::isPublicActivity));
       ...
       *   - .getCount()   → 개수
       */
      IntSummaryStatistics stats = activities
            .stream()
            .collect(Collectors.summarizingInt(LearningActivity::getMinutes)); // TODO 2: 위 힌트를 참고해 완성하세요
       ...
       */
      int totalMinutes = activities
            .stream()
            .mapToInt(a -> a.getMinutes())
            .sum(); // TODO 3: 위 힌트를 참고해 완성하세요
       ...
       */
      double avgMinutes = activities
            .stream()
            .mapToInt(LearningActivity::getMinutes)
            .average()
            .getAsDouble(); // TODO 4: 위 힌트를 참고해 완성하세요
       ...
       */
      LearningActivity longestActivity = activities
            .stream()
            .max(Comparator.comparing(LearningActivity::getMinutes))
            .get(); // TODO 5: 위 힌트를 참고해 완성하세요
      ...
      activities.stream()
            .filter(a -> a.getCategory().equals(ActivityCategory.PRACTICE))
            .filter(a -> a.getMinutes() >= 60)
            .filter(a -> a.isPublicActivity())
            .forEach(a
       ...
       */
      Map<ActivityCategory, Map<Boolean, Long>> grouped = activities
            .stream()
            .collect(Collectors.groupingBy((LearningActivity a) -> a.getCategory()
                  , Collectors.groupingBy((LearningActivity a2) -> a2.isPublicActivity()
                        , Collectors.counting()
                  ))); // TODO 7: 위 힌트를 참고해 완성하세요
