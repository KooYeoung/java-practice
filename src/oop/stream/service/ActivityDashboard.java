...
public class ActivityDashboard {
   ...
   public ActivityDashboard(List<LearningActivity> activities) {
      if(activities == null)throw new IllegalArgumentException("학습 활동 목록은 null일 수 없습니다.");
   ...
   * */
   ...
   /*
   * 보고서 출력기
   * 외부 클래스(ActivityDashboard)가 가지고 잇는 activities 배열에 접근해야 하기 때문에
   * static을 붙이지 않은 멤버 내부 틀래스
   * */
   ...
   public Map<ActivityCategory, List<LearningActivity>> groupByCategory(){
      ...
      for (LearningActivity activity : activities) {
         ActivityCategory category = activity.getCategory();

         // 해당 카테고리가 Map에 없으면 빈 List를 먼저 만들어서 put 한다.
         if(!result.containsKey(category)) result.put(category, new ArrayList<>());

         List<LearningActivity> list = result.get(category);
         list.add(activity);
